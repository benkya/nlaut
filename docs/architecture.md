...
| MVP | web 执行器+数据工厂+HTML 报告+CLI+演示被测系统 | 一条命令端到端 | ✅ 2026-09-23 |
| M1-半 | 口述→IR 生成协议（生成层=对话 AI，见 verbal-to-ir-protocol.md） | 1 条真实口述全流程跑通 | ✅ 2026-09-23（tc_login_005） |
| M1 | `pipeline/` 框架内自动生成（接模型 API）+ 意图澄清 + RAG | 3 条真实口述全自动生成 | 待做 |
| M2 | IR → pytest/Playwright 代码生成 + 验证 Loop | 生成用例可直接执行、失败自动回传修正 | 待做 |
| M3 | 判定结果归因接入报告 + 证据链完整化 | 报告含归因与完整证据包 | 待做 |
| M4 | 数据工厂快照回滚 + RAG 历史用例库 + Electron 通道 | 数据可构造可还原，检索命中率验证 | 待做 |
